...
export default class Renderer {
  ...
  commit() {
    ...
    this.layers.forEach((layer, layerIndex) => {
      let layerEl = this.layerElements[layer.name]
      ...
      if (!layerEl) {
        ...
        layerEl.style.zIndex = layerIndex.toString()

        document.getElementById('asc-engine-layer-container').appendChild(layerEl)
        this.layerElements[layer.name] = layerEl
      }
      ...
      for (let op of layer.operations) {
        ...
        if (op.delete === true) {
          layerEl.removeChild(document.getElementById(`asc-engine-tile-${op.tile.id}`))
        }
